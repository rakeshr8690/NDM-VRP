...
from qgis.PyQt.QtWidgets import QAction, QDialog, QTableWidgetItem, QDialog, QVBoxLayout, QLineEdit, QPushButton, QLabel, QFormLayout, QFileDialog, QTableWidget # type: ignore

from qgis.core import QgsField, QgsFields, QgsVectorLayer, QgsWkbTypes, QgsProject # type: ignore
from PyQt5.QtCore import QVariant # type: ignore
from qgis.core import QgsField, QgsFields, QgsVectorLayer, QgsWkbTypes, QgsProject, QgsCoordinateReferenceSystem # type: ignore
from qgis.core import QgsCoordinateTransform, QgsCoordinateReferenceSystem, QgsProject # type: ignore
from qgis.PyQt.QtCore import Qt # type: ignore
from qgis.utils import iface # type: ignore
from qgis.utils import iface # type: ignore
from PyQt5.QtWidgets import QTableWidgetItem, QLineEdit, QMessageBox # type: ignore
from PyQt5.QtWidgets import QMessageBox # type: ignore
import pandas as pd # type: ignore
from qgis.core import QgsVectorLayer, QgsProject, QgsWkbTypes, QgsCoordinateReferenceSystem, QgsField # type: ignore
...
from .NDMVRP_dialog import NDMVRPDialog # type: ignore
import os.path
import pandas as pd # type: ignore
...
from .shortestpath_algo import shortestpath # type: ignore
from .grip_csv import grip # type: ignore
...
class MatrixInputDialog(QDialog):
    def __init__(self, vehicle_df, cargo_df):
        ...
        cargo_headers = [str(col) for col in cargo_df['Type']]
        ...
        self.table.setHorizontalHeaderLabels(['Vehicle Type'] + cargo_headers + ['Remarks/Comments'])  # Add new header
    ...
    def get_matrix_data(self):
        ...
        matrix_df = pd.DataFrame(matrix_data, columns=['Vehicle Type'] + [str(col) for col in self.cargo_df['Type']] + ['Remarks/Comments'])  # Add new column
...
class FormDialogClass(QDialog):
    def __init__(self, fields):
        ...
        for field in fields:
            ...
            if field == "Vehicle Network Compatibility (OSM)":
                combo_box = QComboBox(self)
                combo_box.addItems(["Net 1", "Net 2", "Net 3"])
                self.layout.addRow(label, combo_box)
                self.field_widgets[field] = combo_box
            elif field in ["Must vehicles of this type finally return to their respective starting depots?",
                ...
                self.field_widgets[field] = checkbox
            else:
    ...
    def get_field_values(self):
        ...
        for field, widget in self.field_widgets.items():
            if isinstance(widget, QCheckBox):
                field_values[field] = 1 if widget.isChecked() else 0
            elif isinstance(widget, QComboBox):
                field_values[field] = widget.currentText()
            else:
                field_values[field] = widget.text()
        return field_values
...
class NDMVRP:
    ...
    def __init__(self, iface):
        ...
        # iniatially we keep compatibility_matrix_generated False--------Shishupal&Pankaj
        self.compatibility_matrix_generated = False
        ...
        self.vehicle_fields = [
            "Vehicle Type", "Description", "Weight Capacity", "Volume Capacity", "Vehicle Network Compatibility (OSM)", "Must vehicles of this type finally return to their respective starting depots?", "Remarks / Comments", "Multimodal Compatibility for Air", "Multimodal Compatibility for Water", "Multimodal Compatibility for Rail", "Multimodal Compatibility for Road"
        ...
        # Create an instance of FormDialogClass with the new fields list - Pickup / Delivery, Type, Description	Unit Weight	Unit Volume	Remarks / Comments
        # auto add pickup / delivery in table
        self.cargo_fields = [
            "Pickup / Delivery", "Type", "Description", "Unit Weight", "Unit Volume", "Remarks / Comments"
        ]
        ...
        self.depot_table_fields = [
            "Sl. No.", "Description", "Latitude", "Longitude", "Multimodal Compatibility for Air", "Multimodal Compatibility for Water",
            "Multimodal Compatibility for Rail", "Multimodal Compatibility for Road", "Remarks/Comments"]
    ...
    def generate_matrix_data(self):
        #now change the compatibility_matrix_generated to True
        self.compatibility_matrix_generated = True
        ...
        self.pickup_ids = [
            id for id in self.cargo_df[self.cargo_df['Pickup / Delivery'] == 'Pickup']['Type']]
        ...
        self.delivery_ids = [
            id for id in self.cargo_df[self.cargo_df['Pickup / Delivery'] == 'Delivery']['Type']]
    ...
    def clickHandler(self, point):
        # if compatibility_matrix_generated is False then this if is executed and Return 
        if not self.compatibility_matrix_generated:
            message_box = QMessageBox()
            message_box.setIcon(QMessageBox.Warning)
            message_box.setText("Please generate Compatibility Matrix before proceeding to Location Details.")
            message_box.setWindowTitle("Compatibility Matrix Required")
            message_box.exec_()
            return
        ...
        depot_dailog_fields = [
            "Description", "Multimodal Compatibility for Air", "Multimodal Compatibility for Water",
            "Multimodal Compatibility for Rail", "Multimodal Compatibility for Road", "Remarks/Comments"] + self.button_id_mapping[self.button_type]
    ...
    def addNoDescriptionLatLong(self, size, description, button_type, field_values):
        ...
        row_data["Vertex Category"] = vertex_category  # Add Vertex Category here
        ...
        for field in ["Multimodal Compatibility for Air", "Multimodal Compatibility for Water",
                      "Multimodal Compatibility for Rail", "Multimodal Compatibility for Road", "Remarks/Comments"]:
    ...
    def save_all_data(self):
        ...
        QMessageBox.information(None, "Success", "Files have been extracted to output path.")
    ...
    def generate_travel_cost_tables(self):
        ...
        road_layers = ["road layer 1.gpkg", "road layer 2.gpkg", "road layer 3.gpkg"]
        output_csv_names = ["Travel_Times_for_VT1.csv", "Travel_Times_for_VT2.csv", "Travel_Times_for_VT3.csv"]
        output_layer_names = ["Travel Time for Vehicle Type 1", "Travel Time for Vehicle Type 2", "Travel Time for Vehicle Type 3"]
    ...
    # run vrp button function - using exec run grip_csv.py file when the run_vrp_pb is clicked

    def run_vrp(self):
        # Get the current directory
        current_directory = os.path.dirname(os.path.realpath(__file__))
        # Get the path of the file in the current directory
        file_path = os.path.join(current_directory, "grip_csv.py")
